...
- An rtuple is a tuple of the form ``(subj, filler, obj)``, where ``subj``
  and ``obj`` are pairs of Named Entity mentions, and ``filler`` is the string
  of words occurring between ``sub`` and ``obj`` (with no intervening NEs).
  Strings are printed via ``repr()`` to circumvent locale variations in
  rendering utf-8 encoded strings.
...
def _expand(class_):
    ...
    """
    try:
        return SHORT2LONG[class_]
    except KeyError:
        return class_

def class_abbrev(class_):
    """
...
def list2sym(lst):
    ...
    sym = sym.lower()
    entity = re.compile("&(\w+?);")
    sym = entity.sub(descape_entity, sym)
    sym = sym.replace('.', '')
    return sym
...
def semi_rel2reldict(pairs, window=5, trace=False):
    ...
    :type window: int
    :return: 'relation' dictionaries whose keys are 'lcon', 'subjclass',\
    'subjtext', 'subjsym', 'filler', objclass', objtext', 'objsym' and 'rcon'
    ...
    while len(pairs) > 2:
        ...
        if trace:
            print("({}({}, {})".format(reldict['untagged_filler'],
                                       reldict['subjclass'], reldict['objclass']))
...
def extract_rels(subjclass, objclass, doc, corpus='ace', pattern=None, window=10):
    ...
    :param subjclass: class or list of classes of the subject Named Entity.
    :type subjclass: str or list(str)
    :param objclass: class or list of classes of the object Named Entity.
    :type objclass: str or list(str)
    ...
    :rtype: list(defaultdict)
    """
    ne_classes = NE_CLASSES[corpus]

    def expand(classes):
        """
        Replace abbreviated classes by their expanded variants
        """
        results = []
        if isinstance(classes, str):
            classes = [classes]
        for class_ in classes:
            if class_ in ne_classes:
                results.append(class_)
            elif _expand(class_) in ne_classes:
                results.append(_expand(class_))
            else:
                raise ValueError("Your value for the type has not been \
                recognized: {}".format(classes))
        return results

    subjclass = set(expand(subjclass))
    objclass = set(expand(objclass))
...
def rtuple(reldict, lcon=False, rcon=False):
    ...
    """
    items = [class_abbrev(reldict['subjclass']), reldict['subjtext'],
             reldict['filler'], class_abbrev(reldict['objclass']), reldict['objtext']]
...
def ne_chunked():
    ...
    rels = []
    for i, sent in enumerate(treebank.tagged_sents()[:1500]):
        sent = ne_chunk(sent)
